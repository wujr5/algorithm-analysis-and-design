...
ii)对重复产生的子节点D，比较从起点节点经新路径到达该点的代价与旧路   
径到该点的代价，若不小于(即cost(D新)>=cost(D旧))，则删除此节点，   
否则对D及相关节点的父指针及cost值进行调整(以新指针及新cost值取代之)
...
炸迷宫问题
```cpp
有一个N*N迷宫，每一格或是空，或者是实，如果有一人位于迷宫的一空格(x,y)中，   
则他仅能到达相邻的空格(指上下左右).现有一人从(1,1)始点出发，目标是(N,N)，   
他随身带着K个炸弹(0<=K<=N)，一个炸弹的威力能把与他相邻的一个实格炸成空格.
